...
describe('Stack', () => {
  ...
  context('pop()', () => {
    it('returns the item at the top of the stack', () => {
      const myStack = new Stack()
      myStack.push('foo')
      expect( myStack.pop() ).to.deep.equal('foo')
    ...
    it('decrements the length of the stack', () => {
      ...
      myStack.push('bar')
      myStack.push('pow')
      expect(myStack.length()).to.deep.equal(3)
      myStack.pop()
      expect(myStack.length()).to.deep.equal(2)
      myStack.push('ping')
      expect(myStack.length()).to.deep.equal(3)
      expect(myStack.peek()).to.deep.equal('ping')
    })
    ...
    it('deletes the item at the top of the stack', () => {
      ...
      myStack.push('bar')
      myStack.pop()
      expect(myStack.peek()).to.deep.equal('foo')
    })
  })
  ...
  context('peek()', () => {
    it('returns the item at the top of the stack', () => {
      ...
      myStack.push('foo')
      myStack.push('bar')
      expect( myStack.peek() ).to.deep.equal('bar')
    })
